...
class ProposalModule(nn.Module):
  ...
  def forward(self, features, pos_anchor_coord=None, \
    ...
    # Replace "pass" statement with your code
    A = self.num_anchors
    out = self.pred_layer(features).reshape(-1,A,6,7,7)
    if mode == 'train':
        anchor_idx = torch.cat((pos_anchor_idx[:,None], neg_anchor_idx[:,None])).squeeze()
        offsets = self._extract_anchor_data(out[:,:,2:,:,:], pos_anchor_idx)
        proposals = GenerateProposal(pos_anchor_coord[None,None,None,:], offsets[None,None,None,:], \
            method='FasterRCNN').reshape(-1,4)
        conf_scores = self._extract_anchor_data(out[:,:,:2,:,:], anchor_idx)
    if mode == 'eval':
        offsets = out[:,:,2:,:,:]
        conf_scores = out[:,:,:2,:,:]
    ##############################################################################
...
class RPN(nn.Module):
  ...
  def forward(self, images, bboxes, output_mode='loss'):
    ...
    out = self.prop_module(features)
    grid = GenerateGrid(B)
    anchors = GenerateAnchor(self.anchor_list.to(images.device), grid)
    iou_mat = IoU(anchors,bboxes)
    activated_anc_ind, negative_anc_ind, _, GT_offsets, GT_class, activated_anc_coord, _ \
    ...
    conf_scores, offsets, proposals = self.prop_module(features, activated_anc_coord, \
          activated_anc_ind, negative_anc_ind)
  ...
  def inference(self, images, thresh=0.5, nms_thresh=0.7, mode='RPN'):
    ...
    with torch.no_grad():
        ...
        grid = GenerateGrid(B)
        anchors = GenerateAnchor(self.anchor_list.to(images.device), grid)
        conf_scores, offsets = self.prop_module(features) # B*A*D*H*W
        conf_scores = conf_scores.softmax(2)[:,:,:1,:,:]
        ...
        for i in range(B):
            ...
            # import pdb;pdb.set_trace()
            keep = torchvision.ops.nms(proposal, score, iou_threshold=nms_thresh)
            final_proposals.append(proposal[keep])
            final_conf_probs.append(score[keep].unsqueeze(1))
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
    if mode == 'RPN':
...
class TwoStageDetector(nn.Module):
  ...
  def forward(self, images, bboxes):
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    rpn_loss, conf_scores, proposals, features, GT_class, pos_anchor_idx, \
        anc_per_img = self.rpn(images, bboxes, output_mode='all')
    batch_ind = (pos_anchor_idx/anc_per_img).long()
    proposals = torch.cat((batch_ind[:,None], proposals),dim = 1)
    out = torchvision.ops.roi_align(features,proposals,(self.roi_output_w, self.roi_output_h))
    out = out.mean((2,3))
    out = self.cls_layer(out.view(out.size(0), -1))
    cls_loss = nn.functional.cross_entropy(out, GT_class)
  ...
  def inference(self, images, thresh=0.5, nms_thresh=0.7):
    ...
    out = torchvision.ops.roi_align(features,final_proposals,(self.roi_output_w, self.roi_output_h))
    out = out.mean((2,3))
    out = self.cls_layer(out.view(out.size(0), -1)).argmax(1, keepdims = True)
    list_of_num = []
    for pro_per_img in final_proposals:
        number = pro_per_img.shape[0]
        list_of_num.append(number)
    final_class = torch.split(out, list_of_num, dim=0)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
