...
def GenerateAnchor(anc, grid):
  ...
  shift = anc/2 #shift size from center to tl and nr
  tl = grid.unsqueeze(1) - shift[None,:,None,None,:]
  br = grid.unsqueeze(1) + shift[None,:,None,None,:]
...
def GenerateProposal(anchors, offsets, method='YOLO'):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  sizes = anchors[:,:,:,:,2:4] - anchors[:,:,:,:,0:2]
  centers = (anchors[:,:,:,:,2:4] + anchors[:,:,:,:,0:2])/2
  proposals = torch.cat((centers, sizes), dim = 4) # proposal with xc,yc,w,h
  pproposals = proposals.clone() # aviod inplace operation
  if method == 'YOLO':
      pproposals[:,:,:,:,0:2] = proposals[:,:,:,:,0:2] + offsets[:,:,:,:,0:2]
      pproposals[:,:,:,:,2:4] = pproposals[:,:,:,:,2:4]*torch.exp(offsets[:,:,:,:,2:4])
  elif method == 'FasterRCNN':
      pproposals[:,:,:,:,0:2] = proposals[:,:,:,:,0:2] + offsets[:,:,:,:,0:2]*proposals[:,:,:,:,2:4]
      pproposals[:,:,:,:,2:4] = proposals[:,:,:,:,2:4] * torch.exp(offsets[:,:,:,:,2:4])
  tls = pproposals[:,:,:,:,0:2] - pproposals[:,:,:,:,2:4]/2
  brs = pproposals[:,:,:,:,0:2] + pproposals[:,:,:,:,2:4]/2
  proposals = torch.cat((tls, brs), dim = 4) # proposal with tl, br
  ##############################################################################
  #                               END OF YOUR CODE                             #
  ##############################################################################
...
class PredictionNetwork(nn.Module):
  ...
  def forward(self, features, pos_anchor_idx=None, neg_anchor_idx=None):
    ...
    if pos_anchor_idx != None:
        anchor_idx = torch.cat((pos_anchor_idx[:,None], neg_anchor_idx[:,None])).squeeze()
        anchor_data = out[:,:(5*A),:,:].reshape(B,A,5,7,7)[:,:,1:,:,:]
        offsets = self._extract_anchor_data(anchor_data, pos_anchor_idx)
        offsets[:,:2] = torch.sigmoid(offsets[:,:2]) - 0.5
        all_scores = out[:,(5*A):(5*A+C),:,:]
        class_scores = self._extract_class_scores(all_scores, pos_anchor_idx)
        all_conf_scores = out[:,0:(5*A):5,:,:].contiguous().view(-1)
        conf_scores = all_conf_scores[anchor_idx,None]
        conf_scores = conf_scores.sigmoid()
    else:
        offsets = out[:,:(5*A),:,:].reshape(B,A,5,7,7)[:,:,1:,:,:]
        offsets[:,:,:2,:,:] = torch.sigmoid(offsets[:,:,:2,:,:]) - 0.5
        class_scores = out[:,(5*A):(5*A+C),:,:]
        conf_scores = out[:,0:(5*A):5,:,:]
...
class SingleStageDetector(nn.Module):
  ...
  def forward(self, images, bboxes):
    ...
    activated_anc_ind, negative_anc_ind, GT_conf_scores, GT_offsets, GT_class, \
        _ ,_ = ReferenceOnActivatedAnchors(anchors, bboxes, grid, iou_mat, pos_thresh=0.7, neg_thresh=0.2, method='YOLO')
    conf_scores, offsets, class_prob = self.pred_network.forward(features, activated_anc_ind, negative_anc_ind)
  ...
  def inference(self, images, thresh=0.5, nms_thresh=0.7):
    ...
    with torch.no_grad():
        ...
        class_scores = class_scores.argmax(1, keepdims = True)
        proposals = GenerateProposal(anchors, offsets, method='YOLO')
        for i in range(B):
            conf_score = conf_scores[i]
            proposal = proposals[i]
            class_score = class_scores[i]
            conf_score = conf_score.reshape(-1)
            # import pdb;pdb.set_trace()
            ind = torch.nonzero((conf_score > thresh), as_tuple=True)[0]
            proposal = proposal.reshape(-1, 4)[ind]
            score = conf_score[ind]
            keep = nms(proposal, score, iou_threshold=nms_thresh)
            final_proposals.append(proposal[keep])
            final_conf_scores.append(score[keep].unsqueeze(1))
            class_score = self.pred_network._extract_class_scores(class_score[None,:], ind)
            final_class.append(class_score[keep])
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def nms(boxes, scores, iou_threshold=0.5, topk=None):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  remain_mask = torch.ones(len(scores)).to(device = boxes.device).bool()
  highest_mask = torch.zeros(len(scores)).to(device = boxes.device).bool()
  while remain_mask.sum() > 0:
    highest_score = scores[remain_mask].max()
    highest_mask |= (scores == highest_score)
    highest_box = boxes[scores == highest_score]
    iou_mat = IoU(highest_box[:,None,None,None,:], boxes[None,:]).squeeze()
    remain_mask &= (iou_mat <= iou_threshold)
  keep = torch.nonzero(highest_mask, as_tuple=True)[0]#.squeeze()
  if topk is None:
    pass
  else:
    keep = keep[:topk]
