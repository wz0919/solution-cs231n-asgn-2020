...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(x.shape[1]):
      hidden_step, cache_step = rnn_step_forward(x[:,i,:], h0, Wx, Wh, b)
      h.append(hidden_step[None,:])
      cache.append(cache_step)
      h0 = hidden_step
    h = torch.cat(h).permute(1,0,2)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
    return h, cache
...
class CaptioningRNN(nn.Module):
    ...
    def __init__(self, word_to_idx, input_dim=512, wordvec_dim=128,
        ...
        self.device = device
        self.dtype = dtype
        # self.cnn = FeatureExtractor(pooling=True, verbose=False, device=device, dtype=dtype).mobilenet
        self.fc1 = nn.Linear(1280, hidden_dim).to(device=device, dtype=dtype)
        ...
        if self.cell_type == 'rnn':
            ...
            self.conv = torch.nn.Conv2d(1280, hidden_dim,(1,1)).to(device=device, dtype=dtype)
        ...
        self.fc2 = nn.Linear(hidden_dim, vocab_size).to(device=device, dtype=dtype)
    ...
    def forward(self, images, captions):
        ...
        c_in = self.we.forward(captions_in)
        out = self.feature_extractor.extract_mobilenet_feature(images)
        # out = self.cnn(images).squeeze()
        if self.cell_type != 'attention':
            out = self.fc1(out)
        if self.cell_type == 'attention':
            out = self.conv(out)
        out = self.rnn.forward(c_in, out)
        out = self.fc2(out)
    ...
    def sample(self, images, max_length=15):
        ...
        # Replace "pass" statement with your code
        images = images.to(device=self.device, dtype=self.dtype)
        c_in = torch.tensor(self._start).repeat(images.shape[0])
        # h = self.cnn(images).squeeze()
        h = self.feature_extractor.extract_mobilenet_feature(images).squeeze()
        if self.cell_type != 'attention':
            h = self.fc1(h)
        ...
        for i in range(max_length):
            c_in = self.we.forward(c_in)
            if self.cell_type == 'rnn':
                h = self.rnn.step_forward(c_in, h)
            elif self.cell_type == 'lstm':
                h, c = self.rnn.step_forward(c_in, h, c)
            elif self.cell_type == 'attention':
                attn, attn_weights = dot_product_attention(h, A)
                h, c = self.rnn.step_forward(c_in, h, c, attn)
                attn_weights_all[:,i] = attn_weights
            out = self.fc2(h)
            c_in = torch.argmax(out, dim = 1)
            # import pdb;pdb.set_trace()
            captions[:,i] = c_in
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    h = []
    for i in range(x.shape[1]):
      h0, c0 = lstm_step_forward(x[:,i,:], h0,c0, Wx, Wh, b)
      h.append(h0[None,:])
    h = torch.cat(h).permute(1,0,2)
    ##############################################################################
    #                               END OF YOUR CODE                             #
...
def dot_product_attention(prev_h, A):
    ...
    Matt = (torch.bmm(h_tilt, A_tilt).div(sqrt(H))).reshape(N, -1)
    attn_weights = nn.functional.softmax(Matt, dim = 1)
    # import pdb; pdb.set_trace()
    attn = torch.bmm(A_tilt, attn_weights.unsqueeze(2)).squeeze()
...
def attention_forward(x, A, Wx, Wh, Wattn, b):
    ...
    for i in range(x.shape[1]):
      ...
      h0, c0 = lstm_step_forward(x[:,i,:], h0,c0, Wx, Wh, b, attn, Wattn)
      h.append(h0[None,:])
    h = torch.cat(h).permute(1,0,2)
    ##############################################################################
    #                               END OF YOUR CODE                             #
