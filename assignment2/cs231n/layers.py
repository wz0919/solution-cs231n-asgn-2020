...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        running_mean = momentum * running_mean + (1 - momentum) * sample_mean
        running_var = momentum * running_var + (1 - momentum) * sample_var
        cache = x_n, gamma, np.sqrt(sample_var + eps)
...
def batchnorm_backward(dout, cache):
    ...
    #x^hat_i = (I_n - 1_n*1_n^T/n)*xi/sqrt(x_i^T(I_n - 1_n*1_n^T/n)x_i/n + e)
    #x_i \in R^{n*D}
    #dx_i = (px^hat_i/px_i)^T*dx^hat_i/std
    #px^hat_i/px_i is in R^(n*n), represent the jacobian of x^hat_i w.r.t x_i
    #std = np.sqrt(sample_var + eps)
    term_1_muti = dx_hat/ _std
    mean_1 = np.mean(term_1_muti, axis = 0)
    term_1 = term_1_muti - mean_1
    term_2_muti = np.sum(x_n * term_1_muti, axis = 0)
    term_2 = x_n * term_2_muti / x_n.shape[0]
    dx = term_1 - term_2
...
def layernorm_backward(dout, cache):
    ...
    dsigma = -0.5 * np.sum(dx_hat * x_n, axis= 0 ) / _std**2
    dmu = -np.sum(dx_hat / _std, axis= 0)
    dx = (dx_hat / _std + 2.0 * dsigma * x_n * _std / N + dmu /N).T
...
def conv_backward_naive(dout, cache):
    ...
    N, C_out, W_out, H_out = dout.shape
    F, C, HH, WW = weights.shape
    padded_x = np.pad(x, ((0,0),(0,0),(pad,pad),(pad,pad)))
...
def max_pool_backward_naive(dout, cache):
    ...
    for h in range(H_out):
      for w in range(W_out):
        ...
        part_x = x[:,:,h*stride:h*stride+pool_height, w*stride:w*stride+pool_width]
        dx[:,:,h*stride:h*stride+pool_height,w*stride:w*stride+pool_width] += (part_x\
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    #import pdb;pdb.set_trace()
    x = x.reshape(n,G,-1)
    sample_mean = np.mean(x, axis = 2, keepdims = True)
    sample_var = np.var(x, axis = 2, keepdims = True)
    x_n = (x - sample_mean)/(np.sqrt(sample_var + eps))
    x_n = x_n.reshape(n,c,h,w)
    out = gamma * x_n + beta
    cache = x_n, gamma, G, np.sqrt(sample_var + eps)
...
def spatial_groupnorm_backward(dout, cache):
    ...
    x_n, gamma,G, _std = cache
    x_n_shape = x_n.shape
    N = x_n.shape[0]
    dgamma = np.sum(dout * x_n, axis = (0,2,3), keepdims = True)
    dbeta = np.sum(dout, axis = (0,2,3), keepdims = True)
    x_n = x_n.reshape(N,G,-1)
    dx_hat = (dout * gamma).reshape(N,G,-1).T
    x_n = x_n.T
    N = x_n.shape[0]
    _std = _std.T
    dsigma = -0.5 * np.sum(dx_hat * x_n, axis= 0 ) / _std**2
