...
def affine_norm_relu_forward(x, w, b, gamma, beta, norm, **kwargs):
  assert norm == 'batchnorm' or 'layernorm', 'unknown normlization method'
  a, fc_cache = affine_forward(x, w, b)
  if norm == 'batchnorm':
    bn_param = kwargs.pop("bn_param")
    a_norm, norm_cache = batchnorm_forward(a, gamma, beta, bn_param)
  if norm == 'layernorm':
    bn_param = kwargs.pop("ln_param")
...
def affine_norm_relu_backward(dout, cache, norm):
    ...
    if norm == 'batchnorm':
      da, dgamma, dbeta = batchnorm_backward_alt(da_norm, norm_cache)
    if norm == 'layernorm':
