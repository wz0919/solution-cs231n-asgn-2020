...
def gram_matrix(features, normalize=True):
    ...
    reshaped = features.reshape((N, C,-1))
    gram = torch.zeros((N,C,C))
    for i in range(N):
      # import pdb; pdb.set_trace()
      gram[i] = reshaped[i] @ reshaped[i].T
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for i in range(len(style_layers)):
      ...
      feat = feats[l]
      loss += w*torch.sum((gram_matrix(feat)-target)**2)
...
def tv_loss(img, tv_weight):
    ...
    _, _, H, W = img.shape
    v_loss = torch.sum((img[:,:,:(H-1),:] - img[:,:,1:H,:])**2)
    h_loss = torch.sum((img[:,:,:,:(W-1)] - img[:,:,:,1:W])**2)
    loss = tv_weight*(v_loss + h_loss)
