...
def discriminator_loss(logits_real, logits_fake):
    ...
    N = logits_real.shape
    data = torch.vstack((logits_real, logits_fake))
    label = torch.vstack((torch.ones(N), torch.zeros(N))).type(dtype)
    loss = bce_loss(data, label)*2
...
def get_optimizer(model):
    ...
    optimizer = optim.Adam(model.parameters(), 1.0e-3, [0.5, 0.999])
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    loss = ((scores_real - 1.)**2/2).mean() + ((scores_fake)**2/2).mean()
